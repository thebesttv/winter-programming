...
const int MAXV=520;
const int MAXE=150*300+150+300;
...
int main(void){
  scanf("%d%d",&m,&n);
  int t,sum=0;
  FORR(i,1,m) scanf("%d",&t), mf.addEdge(S,i,t), sum+=t;
  FORR(i,1,n) scanf("%d",&t), mf.addEdge(i+m,T,t);
  FORR(i,1,m) FORR(j,1,n) mf.addEdge(i,j+m,1);

  if(mf.maxFlow(S,T)!=sum) {printf("%d\n",0); return 0;}
  printf("%d\n",1);
  FORR(i,1,m){
    FOR(j,0,mf.G[i].size()){
      Edge &e=mf.edge[mf.G[i][j]];
      if(e.cap && e.v>m && e.v!=T && e.flow>0) printf("%d ",e.v-m);
    }
    printf("\n");
  }
